  ...
  /*      
   ...
   /*********************************************************************************************/ 
    ...
    //3- get first hotel     
     ...
     //4- add first hotel    
        HotelInfoEntity hotinf= listHotel.getHotel().get(0);;
        hotinf.setCreditCardGuarantee(true);
        hotinf.setHotelAddress("elektrovej");
        hotinf.setHotelLocation("DTU");
        hotinf.setHotelName("campus village");
        hotinf.setPrice(200);
        hotinf.setResServiceName("service");
                                  
        Link addHotel1 = listHotel.getLinkByRelation(HOTELADD_RELATION);
        responseadd = client.resource(addHotel1.getUri())
                .accept(MEDIATYPE_XML)
                .type(MEDIATYPE_XML)
                .put(StatusRepresentation.class,hotinf);
         getIti = responseadd.getLinkByRelation(ITINERARY_RELATION);
         iti = client.resource(getIti.getUri())
                .get(ItineraryRepresentation.class);
            // we check if the flight was added to the itinerary
        ...
        listFlight.getFlight().get(0).setBookingNumber(1515);
      ...
      //7- book itinerary 
       ...
       Link bookItinerarylink = iti.getLinkByRelation(BOOKITINERARY_RELATION);
       StatusRepresentation responseBook= new StatusRepresentation();
       try {
           responseBook = client.resource(bookItinerarylink.getUri())
                .accept(MEDIATYPE_XML)
                .type(MEDIATYPE_XML).
                put(StatusRepresentation.class,creditcard);
           fail();
        ...
        assertEquals("cancelled",iti.getItinerary().getHotelList().get(0).getStatus());
        assertEquals("unconfirmed",iti.getItinerary().getFlightsList().get(1).getStatus());
 }
   ...
   //1- get first flight  
    ...
    //2- add first flight    
        ...
        Link getHotel1 = iti.getLinkByRelation(GETHOTEL_RELATION);
        String link2 = String.format("%s?arrival=%s&departure=%s&city=%s", getHotel1.getUri().toString(),"15/01/2014","4/11/2014","Lorient");
        HotelRepresentation listHotel =client.resource(link2).accept(MEDIATYPE_XML).get(HotelRepresentation.class);
       assertEquals(2,listHotel.getHotel().size());
    ...
    //5- get second hotel     
     ...
     //6- add second hotel    
        ...
        StatusRepresentation resultCancelItinerary = client.resource(cancelItineraryLink.getUri()).accept(MEDIATYPE_XML).
                type(MEDIATYPE_XML).put(StatusRepresentation.class,creditcard);
        assertEquals(1,iti.getItinerary().getFlightsList().size());
        assertEquals(2,iti.getItinerary().getHotelList().size());
        assertEquals("Itinerary canceled",resultCancelItinerary.getStatus()); 
